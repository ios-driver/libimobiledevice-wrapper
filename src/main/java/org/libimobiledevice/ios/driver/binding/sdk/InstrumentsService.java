...
public class InstrumentsService {
  ...
  private final ImobiledeviceInstrumentsLibrary.uiautomation_message_handler_t msgHandler= new Handler();
  ...
  public InstrumentsService(IDeviceSDK device, String bundleId) {
    this.device = device;
    this.bundleId = bundleId;

    // TODO freynaud
    PointerByReference ptr = new PointerByReference();
    ImobiledeviceInstrumentsLibrary.instruments_client_start_service(device.getHandleIdevice(), ptr, "libimobile-java");
    ...
    process_control_client = new ImobiledeviceInstrumentsLibrary.processcontrol_client_t(ptr3.getValue());


    ImobiledeviceInstrumentsLibrary.uiautomation_client_set_message_handler(automation_client, msgHandler);
    ...
    ImobiledeviceInstrumentsLibrary.processcontrol_client_process_identifier_for_bundle_identifier(process_control_client,bundleId,pid_ptr);
    pid = pid_ptr.get(0);
    ...
    ImobiledeviceInstrumentsLibrary.uiautomation_client_configure_launch_environment(automation_client, new PointerByReference());


    ImobiledeviceInstrumentsLibrary.processcontrol_client_launch_suspended_process_with_bundle_identifier(
        process_control_client, bundleId, new PointerByReference(), new PointerByReference(),
        pid_ptr);
    pid= pid_ptr.get(0);

    ImobiledeviceInstrumentsLibrary.processcontrol_client_start_observing_pid(process_control_client,pid);
    ImobiledeviceInstrumentsLibrary.processcontrol_client_resume_pid(process_control_client,pid);

    ImobiledeviceInstrumentsLibrary.uiautomation_client_start_agent_for_app_with_pid(automation_client,bundleId,pid);

    // wait for agent ready.
    int res =0;
    while (res!=1){
      try {
        res = ImobiledeviceInstrumentsLibrary.uiautomation_client_get_agent_ready(automation_client);
        Thread.sleep(250);
